...
class OrquestadorFiscal:
    ...
    def ejecutar_auditoria_fiscal_completa(self) -> Dict[str, Any]:
        ...
        logging.info(
            f"[ORQUESTADOR] Iniciando Pipeline Senior para {self.empresa.nombre_empresa} ({self.anio_fiscal})..."
        )
        ...
        self.db.query(ValidacionFiscal).filter_by(
        ...
        experta_hallazgos = (
            self.db.query(ValidacionFiscal)
            .filter(
                and_(
                    ValidacionFiscal.empresa_id == self.empresa_id,
        ...
        for h in experta_hallazgos:
            if h.estado != "OK":
                ...
                if h.asiento_propuesto:
                    self.asientos_propuestos.append(
                        {"origen": h.tipo_validacion, "asiento": h.asiento_propuesto}
                    )
            ...
            "socios": [
                {
                    ...
                    "nombre": s.nombre_razon_social,
                    "pct": float(s.porcentaje_participacion),
                }
                for s in self.db.query(Socio)
        ...
        riesgos_prov = validador_rnc.validar_proveedores_criticos(
            self.empresa_id, self.anio_str
        )
        ...
        # ==================== FASE 9: CONSOLIDACIÓN, PLANIFICACIÓN IA Y CÁLCULO FINAL IR-2 ====================
        logging.info("  [Fase 9] Consolidación y Planificación IA...")
        try:
            if resultados["bloqueos"]:
                resultados["estado"] = "Bloqueado"
                resultados["mensaje"] = (
                    "El proceso se detuvo por inconsistencias críticas."
                )
            else:
                self._consolidar_resultados(er_base, costo_ventas, depreciacion)

                # INTEGRACIÓN IA: Planificación Predictiva
                try:
                    from planificador_fiscal import PlanificadorIA

                    planificador = PlanificadorIA(self.db, self.empresa_id, self.anio_fiscal)
                    planificador.actualizar_planificacion_db()
                    proyeccion = planificador.proyectar_cierre_anual()
                    resultados["proyeccion_ia"] = proyeccion
                except ImportError:
                    resultados["proyeccion_ia"] = {
                        "error": "Módulo planificador_fiscal no disponible"
                    }
            ...
            self.db.commit()
            logging.info(
                f"[ORQUESTADOR] Pipeline y Planificación IA completados. Estado: {resultados['estado']}"
            )
    ...
    def _generar_er_base(self):
        ...
        er_prev = (
            self.db.query(EstadoFinanciero)
            .filter_by(empresa_id=self.empresa_id, periodo=self.anio_str)
            .first()
        )
        if er_prev:
            self.db.delete(er_prev)

        ventas = float(
            self.db.query(func.sum(Dgii607.monto_facturado))
            .filter(
                and_(
        ...
        ventas_exentas = float(
            self.db.query(func.sum(Dgii607.monto_exento))
            .filter(
                and_(
                    Dgii607.empresa_id == self.empresa_id,
                    Dgii607.periodo.like(f"{self.anio_str}%"),
        ...
        gastos = float(
            self.db.query(func.sum(Dgii606.monto_facturado))
            .filter(
                and_(
    ...
    def _consolidar_resultados(self, er, costo_ventas, depreciacion):
        ...
        u_operativa = Decimal(str(er.utilidad_bruta)) - Decimal(
            str(er.gastos_operativos)
        )
    ...
    def obtener_dashboard_data(self) -> Dict:
                ...
                hallazgos.append(
                    {
                        ...
                        "descripcion": v.recomendacion_socio
                        or "Se detectó una discrepancia que requiere revisión.",
        ...
        return {
            "resumen": {
                ...
                "renta_neta": res["ir2_data"]["renta_neta_imponible"]
                if res["ir2_data"]
                else 0,
                "isr_estimado": res["ir2_data"]["isr_pagar"] if res["ir2_data"] else 0,
            },
            "planificacion": res.get("proyeccion_ia", {}),
...
def calcular_estado_resultados(db: Session, rnc: str, anio: int):
    """
    Wrapper de compatibilidad para tests y scripts anteriores.
    Crea un `OrquestadorFiscal`, genera el EstadoFinanciero base y lo devuelve.
    """
    orq = OrquestadorFiscal(db, rnc, anio)
    return orq._generar_er_base()


def ejecutar_auditoria_fiscal(db: Session, rnc: str, anio: int):
    """
    Wrapper que ejecuta el pipeline completo de auditoría y devuelve el resultado.
    """
    orq = OrquestadorFiscal(db, rnc, anio)
    return orq.ejecutar_auditoria_fiscal_completa()
