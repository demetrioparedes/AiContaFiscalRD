...
from database import Dgii606, Empresa, EstadoFinanciero, SessionLocal, ValidacionFiscal
...
def main():
    sys.stdout = open(
        r"c:\GEMINI\AiContaFiscalRD\resultado_humo.txt", "w", encoding="utf-8"
    )
    try:
        rnc = "130826552"
        anio = 2025
        ...
        if empresa:
            ...
            db.query(EstadoFinanciero).filter_by(
                empresa_id=emp_id, periodo=anio_str
            ).delete()
            db.query(ValidacionFiscal).filter_by(
                empresa_id=emp_id, periodo=anio_str
            ).delete()
            db.commit()
        ...
        regs_procesados = (
            db.query(Dgii606).filter_by(empresa_id=emp_id, periodo=str(202501)).count()
        )
        regs_anulados = (
            db.query(Dgii606)
            .filter_by(empresa_id=emp_id, periodo=str(202501), anulada=1)
            .count()
        )

        # Consultar Banderas
        banderas = (
            db.query(ValidacionFiscal)
            .filter_by(empresa_id=emp_id, periodo=anio_str)
            .all()
        )

        gastos_sin_ncf = Decimal(
            str(
                db.query(func.sum(Dgii606.monto_facturado))
                .filter(
                    and_(
                        Dgii606.empresa_id == emp_id,
                        Dgii606.periodo.like(f"{anio_str}%"),
                        Dgii606.anulada != 1,
                        or_(
                            Dgii606.ncf == None, Dgii606.ncf == "", Dgii606.ncf == "N/A"
                        ),
                    )
                )
                .scalar()
        ...
        print(
            f"Utilidad operativa:               RD$ {er.utilidad_bruta - er.gastos_operativos:>15,.2f}\n"
        )
        ...
        print(f"RENTA IMPONIBLE:                  RD$ {renta_im:>15,.2f}")
        print(
            f"ISR BRUTO (27%):                  RD$ {renta_im * Decimal('0.27'):>15,.2f}"
        )
        ...
        for b in banderas:
            estado = (
                "VERDE"
                if b.estado == "OK"
                else ("AMARILLO" if b.estado == "ALERTA" else "ROJO")
            )
            print(
                f"[{estado}] {b.tipo_validacion} | Sist: {b.valor_sistema:,.2f} | DGII: {b.valor_dgii:,.2f} | Dif: {b.diferencia:,.2f}"
            )
